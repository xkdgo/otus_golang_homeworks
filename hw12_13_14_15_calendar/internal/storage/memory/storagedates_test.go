...
func TestStorageGetEvents(t *testing.T) {
	...
	t.Run("test storage get events by day, week, month", func(t *testing.T) {
		...
		require.NoError(t, err)
		scheduledEventsForUser, err := memst.GetEventsOnDay("1", testTime)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		scheduledEventsForUser, err = memst.GetEventsOnCurrentWeek("1", testTime)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		scheduledEventsForUser, err = memst.GetEventsOnCurrentMonth("2", testTime)
		require.NoError(t, err)
		require.Equal(t, 27, len(scheduledEventsForUser))
		_, err = memst.GetEventsOnCurrentMonth("3", testTime)
		require.Error(t, err)
	})
}
