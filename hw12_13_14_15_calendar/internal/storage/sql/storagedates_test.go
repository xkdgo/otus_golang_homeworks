...
func TestStorageGetEvents(t *testing.T) {
	...
	t.Run("test storage get events by day, week, month", func(t *testing.T) {
		...
		// 	require.Equal(t, daysInJan, len(sqlst.data))
		// 	require.Equal(t, 1, len(sqlst.userSchedule))
		...
		// 	require.Equal(t, daysInJan+daysInFeb, len(sqlst.data))
		// 	require.Equal(t, 2, len(sqlst.userSchedule))
		// 	testTime, err := time.Parse(timelayout, "2021-01-02")
		// 	require.NoError(t, err)
		// 	scheduledEventsForUser, err := sqlst.ListEventsOnDay("1", testTime)
		// 	require.NoError(t, err)
		...
		// 	require.NoError(t, err)
		// 	scheduledEventsForUser, err = sqlst.ListEventsOnCurrentWeek("1", testTime)
		// 	require.NoError(t, err)
		...
		// 	require.NoError(t, err)
		// 	scheduledEventsForUser, err = sqlst.ListEventsOnCurrentMonth("2", testTime)
		// 	require.NoError(t, err)
		// 	require.Equal(t, 27, len(scheduledEventsForUser))
		// 	_, err = sqlst.ListEventsOnCurrentMonth("3", testTime)
		// 	require.Error(t, err)
	})
}
